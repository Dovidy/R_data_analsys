...
mpg2 <- mpg %>%
  ...
  filter(class=='suv') %>%
  summarise(average = mean(cty, na.rm=TRUE)) %>% slice(1:5)

ggplot(mpg2, aes(x=reorder(manufacturer, average), y=average)) + theme_bw() + geom_bar(stat='identity')
...
ggplot(mpg3, aes(x=class, y=count)) + theme_bw() + geom_bar(stat='identity')

## 5
economics
ggplot(economics, aes(x=date, y=psavert)) + theme_bw() + geom_line()
